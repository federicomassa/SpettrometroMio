...
void main_constz() {
  string gen_name, det_name, root_name, path;
  path = "../Spettrometro_Files/";
  ...
  for (int i = 0; i < 10; i++) {
    ...
    ss << z_val;
    ss >> str_buff;
    gen_name = path + gen_prefix + str_buff + str_dat;
    det_name = path + det_prefix + str_buff + str_dat;
    root_name = path + root_prefix + str_buff + str_root;
