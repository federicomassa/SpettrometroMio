...
//x1+, y1+, x1-, y1-, analogous expression for the 2nd detector
 TMatrixD GetConstraintsVector(Double_t* coord) {
  ...
  void Reconstruction(TH1F* CDA_plus_hist, TH1F* CDA_min_hist, TH1F* CDA_mean_hist, TH1F* kinematic_z_hist, TH1F* iteration_hist, TH1F* iteration_cut_hist, TH1F* iteration_no_hist, TH2F* z_itno_iter_corr_hist, TH1F* kin_iter_diff_hist, TH2F* kin_iter_zz_hist, TH1F* chi2_init_hist, TH2F* chi2_corr_hist, TH1F* chi2_kin_hist, TH1F* chi2_iter_hist, TH2F* z_theta_corr_hist, TH1F* costheta_plus_reco_hist, TH1F* costheta_min_reco_hist, TH1F* tanphi_plus_reco_hist, TH1F* tanphi_min_reco_hist) {
    ...
    pars[3] = impr_measures[2];
    pars[4] = impr_measures[3];
    ...
    if (isValid && chi2_all(impr_measures) <= 16.2662) iteration_cut_hist->Fill(z_k(pars) - K_z); 
    chi2_corr_hist->Fill(K_z - z_k(pars), chi2_all(impr_measures));
    if (TMath::Abs(z_k(pars) - K_z) > 5) chi2_iter_hist->Fill(chi2_all(impr_measures));
    ...
    // Cos(theta) and Tan(phi) resolution

    Double_t norm2_plus;
    norm2_plus = TMath::Power(impr_measures[4]-impr_measures[0],2) + TMath::Power(impr_measures[5]-impr_measures[1],2) + TMath::Power(z2_plus-z1_plus,2);
    Double_t norm_plus = TMath::Sqrt(norm2_plus);
    
    Double_t norm2_min;
    norm2_min = TMath::Power(impr_measures[6]-impr_measures[2],2) + TMath::Power(impr_measures[7]-impr_measures[3],2) + TMath::Power(z2_min-z1_min,2);
    Double_t norm_min = TMath::Sqrt(norm2_min);
    
    Double_t theta_plus_reco, theta_min_reco;
    
    theta_plus_reco = TMath::ACos((z2_plus-z1_plus)/norm_plus);
    theta_min_reco = TMath::ACos((z2_min-z1_min)/norm_min);
    
    costheta_plus_reco_hist->Fill(TMath::Cos(pi_plus_theta)-TMath::Cos(theta_plus_reco));
    costheta_min_reco_hist->Fill(TMath::Cos(pi_min_theta)-TMath::Cos(theta_min_reco));
    
    Double_t phi_plus_reco;
    phi_plus_reco = TMath::ATan((impr_measures[5]-impr_measures[1])/(impr_measures[4]-impr_measures[0]));
    if (phi_plus_reco < 0) phi_plus_reco += 2*TMath::Pi();
    if (TMath::Sign(1.0,impr_measures[4]-impr_measures[0])*TMath::Sign(1.0,TMath::Sin(theta_plus_reco)) < 0) phi_plus_reco += TMath::Pi();
    
    Double_t phi_min_reco;
    phi_min_reco = TMath::ATan((impr_measures[7]-impr_measures[3])/(impr_measures[6]-impr_measures[2]));
    if (phi_min_reco < 0) phi_min_reco += 2*TMath::Pi();
    if (TMath::Sign(1.0,impr_measures[6]-impr_measures[2])*TMath::Sign(1.0,TMath::Sin(theta_min_reco)) < 0) phi_min_reco += TMath::Pi();
   
    tanphi_plus_reco_hist->Fill(TMath::Tan(pi_plus_phi) - TMath::Tan(phi_plus_reco));
    tanphi_min_reco_hist->Fill(TMath::Tan(pi_min_phi) - TMath::Tan(phi_min_reco));
