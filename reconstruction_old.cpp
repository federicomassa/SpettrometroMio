...
void reconstruction(const char* event_name = "events", const char* detector_name = "measures", const char* root_name = "reconstruction", const char* impr_meas_name = "impr_measures") {
  stringstream ss;
  string path = "../Spettrometro_Files/";
  string complete_event_name;
  string complete_detector_name, complete_impr_meas_name;
  ...
  ss << impr_meas_name;
  ss >> complete_impr_meas_name;
  complete_impr_meas_name = path + complete_impr_meas_name + dat_ext;
  ss.clear();
  ...
  ofstream impr_measures_file(complete_impr_meas_name.c_str());
  string str1, str2;
  event ev;
  ...
  TH1F* costheta_plus_reco_hist = new TH1F("costheta_plus_reco_hist","Cos(theta+) Reco Hist;cos(theta+)-cos(thetaReco+)", 1000, 1, 0);
  TH1F* costheta_min_reco_hist = new TH1F("costheta_min_reco_hist","Cos(theta-) Reco Hist;cos(theta-)-cos(thetaReco-)", 1000, 1, 0);
  TH1F* tanphi_plus_reco_hist = new TH1F("tanphi_plus_reco_hist","Tan(phi+) Reco Hist;tan(phi+)-tan(phiReco+)", 10000, -10, 10);
  TH1F* tanphi_min_reco_hist = new TH1F("tanphi_min_reco_hist","Tan(phi-) Reco Hist;tan(phi-)-tan(phiReco-)", 10000, -10, 10);



  //Skip first 5 lines and get imax
  getline(event_file,str1);
  impr_measures_file << str1 << endl;
  ...
  impr_measures_file << endl;
  impr_measures_file << "ev_n" << '\t'
		     << "K_z" << '\t'
		     << "K_p" << '\t'
		     << "p+" << '\t'
		     << "th+" << '\t'
  ...
  while (!event_file.eof() && !detector_file.eof()) {
    ...
    if (dec_no == ev_no) {
      ...
      ev.Reconstruction(impr_measures_file, CDA_plus_hist, CDA_min_hist, CDA_mean_hist, kinematic_z_reco_hist, iteration_hist, iteration_cut_hist, iteration_no_hist, z_itno_iter_corr_hist, kin_iter_diff_hist, kin_iter_zz_hist, chi2_init_hist, chi2_corr_hist, chi2_kin_hist, chi2_iter_hist, z_theta_corr_hist, costheta_plus_reco_hist, costheta_min_reco_hist, tanphi_plus_reco_hist, tanphi_min_reco_hist);
